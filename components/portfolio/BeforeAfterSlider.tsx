...
export default function BeforeAfterSlider({ beforeUrl, afterUrl, beforeAlt, afterAlt, beforeLabel = 'Before', afterLabel = 'After', tagAsGallery = false, reverseDom = false }: Props) {
  ...
  return (
    <div
      ...
      onPointerDown={(e) => { setDragging(true); move(e.clientX); e.currentTarget.setPointerCapture(e.pointerId) }}
      ...
      <div className="absolute inset-0">
        {(reverseDom
          ...
          <img
            ...
            draggable={false}
            style={im.clip ? { clipPath: `inset(0 ${100 - pos}% 0 0)`, zIndex: 1 } : undefined}
          />
        ))}
      </div>

      {/* divider + handle */}
      <div className="absolute top-0 bottom-0 w-[2px] bg-white/90 shadow-[0_0_12px_rgba(0,0,0,0.35)] z-10" style={{ left: `${pos}%` }}>
